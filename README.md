...
We're open to adding pretty much any model that's useful for bio/genomics, as long as we can get it running somewhere. For now, we've started with three models chosen to cover fairly different cases:
...
Evo and Nucleotide Transformer need `torch` + `transformers` (see their `requirement.txt`), plus a GPU if you want anything faster than a crawl — and Evo's weights alone are a multi-gigabyte download from Hugging Face on first run, so make sure you've got disk space and time before kicking it off.

## Starting a single model server (debug)

```bash
cd serveur_evo  # or one of the other two
pip install -r requirement.txt
python evo_server.py       # or nt_server.py / biomistral_server.py
